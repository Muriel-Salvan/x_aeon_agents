describe XAeonAgents::Agents::ReadmeGeneratorAgent do
  it 'does not regenerate the README when using session_id on subsequent calls' do
    stub_doctoc
    stub_readme_generator_run
    run_readme_generator(session_id: 'test-session-123')
    ...
    run_readme_generator(session_id: 'test-session-456')
