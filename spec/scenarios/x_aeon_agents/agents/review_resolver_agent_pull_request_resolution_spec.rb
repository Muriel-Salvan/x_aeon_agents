describe XAeonAgents::Agents::ReviewResolverAgent do
  describe 'pull request resolution' do
    before do
    ...
    context 'with an optional Pull Request number' do
      it 'auto-detects the Pull Request from the current branch' do
        with_github_pr(
          ...
          # No PR number given: should resolve PR #42 from the current branch 'feature-branch'.
          described_class.new(session_id: nil).run(pull_request_number: nil)
